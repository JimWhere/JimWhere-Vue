...
import {createEntry} from "@/api/entry.js";
...
const roomOptions = ref(
    Array.from({ length: 10 }, (_, i) => ({
      label: `방 ${i + 1}`,
      value: String(i + 1)   // 드롭다운 v-model이 문자열일 때 안전
    }))
)
    /* TODO: 방 목록 API 연동 예정 */
...
const addItem = () => {
  ...
  const boxOption = boxOptions.value.find(o => o.value === itemForm.value.boxId)
  ...
  items.value.push({
    ...
    itemName: name,
    boxId: Number(itemForm.value.boxId),
...
const handleSubmit = async () => {
  ...
  const invalidItem = items.value.find(i => !i.boxId)
  if (invalidItem) {
    alert('박스를 선택하지 않은 물품이 있습니다.')
    return
  }
  ...
  const payload = {
    isOwner: 'Y',
    visitPurpose: 'INOUT',
    roomCode: Number(form.value.roomId),
    inOutHistoryRequestList: items.value.map(item => ({
      inOutType: item.movementType,
      inOutName: item.itemName,
      inOutQuantity: item.stockDelta,
      boxCode: Number(item.boxId)   // ✅ 이제 확실히 숫자
    }))
  }
...
// ✅ 토큰 페이지로 전달 (query 방식)
  router.push({
    name: 'EntryQrResponse', // 라우터 name에 맞게 바꾸세요
    query: { qrToken }
  })
  console.log(payload)
}
